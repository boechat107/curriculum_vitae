...
I'm a versatile computer engineer and passionate about developing automation solutions, focusing on creating value and/or reducing costs.
For now, I'm very interested in machine learning (deep neural networks for image processing; natural language processing; predictions) and functional programming languages.

\vspace{.3cm}
Technical experience/knowledge:

\begin{itemize}
\item Clojure, Java, Python, Octave/Matlab, C
\item BoofCV, libSVM (support vector machines), Caffe (deep neural networks)
\item Scrapy
\item Redis, MongoDB, Postgres
\item Linux (Debian, Ubuntu)
\item Docker
\item Git, Vim, Eclipse, Latex
\end{itemize}
...
\item I don't like to perform anything badly
...
\item[] \textit{Software Engineer, Team Leader}
    \hfill \textbf{November 2011 to Present}
    
    \begin{innerlist}
    \item Maintains Captcha breaking project
    \begin{innerlist}
        \item developed image processing routines (Java and BoofCV)
        \item developed SVM models for classification
        \item developed a micro-service based on Redis, Docker and Java
        \item researches state-of-art techniques
    \end{innerlist}
    \item Created and develops Deep Neural Network project for OCR
    \begin{innerlist}
        \item researches state-of-art techniques and applications
        \item develops a micro-service based on RabbitMQ, Docker, Python and Caffe
    \end{innerlist}
    \item Leaded technological and architectural changes of web spiders
    \begin{itemize}
        \item developed modules to facilitate the development/maintenance of spiders
        \item prepared them to use micro-services 
        (Python, Scrapy, RabbitMQ, Redis, Postgres, Docker)
        \item created a micro proxy service using Clojure, Redis, MongoDB and Docker
        \item created a middleware to support compatibility with old spiders
        (Clojure, Postgres, RabbitMQ)
    \end{itemize}
    \end{innerlist}
...
\item Writing: very good
\item Conversation: advanced
\item approximately C1 CEFR
\end{innerlist}
